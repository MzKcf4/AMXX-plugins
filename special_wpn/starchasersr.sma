...
public precache_StarchaserSR()
{
	precache_model("sprites/starchasersr/ef_starchasersr.spr");
	precache_model("sprites/starchasersr/ef_starchasersr_explosion.spr");
	precache_model("sprites/starchasersr/ef_starchasersr_star.spr");
	g_cache_ssr_line = precache_model("sprites/starchasersr/ef_starchasersr_line.spr");
}

public Item_PostFrame_StarchaserSR(id,iEnt)
{
	...
	*/
	...
	if (iButton & IN_ATTACK)
	{
		...
		*/
		...
		new pEntity = Stock_CreateSpriteEntity(id, "sprites/starchasersr/ef_starchasersr_explosion.spr", vVector[0], vVector[0], 0.0, 0.01, MOVETYPE_NONE, ENTCLASS_FIRE);
		...
		set_pev(pEntity, pev_modelindex, engfunc(EngFunc_ModelIndex, "sprites/starchasersr/ef_starchasersr_explosion.spr"));
		set_pev(pEntity, pev_nextthink, get_gametime()+0.01);
		set_pev(pEntity, pev_animtime, 1.0);
		set_pev(pEntity, pev_framerate, 30.0 );
		// SF_SPRITE_STARTON = Repeat , Once = no repeat
		set_pev(pEntity, pev_spawnflags, SF_SPRITE_ONCE )
    	dllfunc(DLLFunc_Spawn, pEntity )
		
		// Seems only for shooting particle effects on the gun bolt
		/*
		...
		*/
		...
		set_pdata_float(iEnt, m_flTimeWeaponIdle, 1.0 + 1.0);
		
		// client_cmd(id, "speak items/equip_nvg.wav");
		set_pev(iEnt, pev_fuser1, get_gametime()+ 1.0 + 1.0);
	}
}