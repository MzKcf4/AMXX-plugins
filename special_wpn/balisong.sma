#define BALISONG_SLASH_SEQ_1 2
#define BALISONG_SLASH_SEQ_2 3

#define BALISONG_STAB_SEQ_1 4
#define BALISONG_STAB_SEQ_2 5
#define BALISONG_STAB_SEQ_3 6

PrimaryAttack_Pre_Balisong(id , iEnt)
{
	static iPrevSlashSeq;
	iPrevSlashSeq = pev(iEnt, pev_iuser1);
	if(iPrevSlashSeq == BALISONG_SLASH_SEQ_1)
	{
		Stock_SetWeaponAnimation(id , BALISONG_SLASH_SEQ_2);
		set_pev(iEnt, pev_iuser1 , BALISONG_SLASH_SEQ_2)
	}
	else
	{
		Stock_SetWeaponAnimation(id , BALISONG_SLASH_SEQ_1);
		set_pev(iEnt, pev_iuser1 , BALISONG_SLASH_SEQ_1)	
	}
}

SecondaryAttack_Pre_Balisong(id, iEnt)
{
	static iPrevStabSeq;
	iPrevStabSeq = pev(iEnt, pev_iuser2);
	if(iPrevStabSeq == BALISONG_STAB_SEQ_1)
	{
		Stock_SetWeaponAnimation(id , BALISONG_STAB_SEQ_2);
		set_pev(iEnt, pev_iuser2 , BALISONG_STAB_SEQ_2)
	}
	else if(iPrevStabSeq == BALISONG_STAB_SEQ_2)
	{
		Stock_SetWeaponAnimation(id , BALISONG_STAB_SEQ_3);
		set_pev(iEnt, pev_iuser2 , BALISONG_STAB_SEQ_3)	
	}
	else
	{
		Stock_SetWeaponAnimation(id , BALISONG_STAB_SEQ_1);
		set_pev(iEnt, pev_iuser2 , BALISONG_STAB_SEQ_1)	
	}
}

SecondaryAttack_Post_Balisong(id, iEnt)
{
Stock_SetWeaponNextAttacks(iEnt , 0.7 , 0.7, 0.7 + 3.0);
}

/*
public WE_JKnife(id,iEnt,iWpnId)
{
